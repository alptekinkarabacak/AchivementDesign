...
namespace Game
{
    ...
    void GameLoop::OnTap(const Interaction::Tap& tap)
    {
         ...
         if(m_data_container.m_player_data_handler.tap_count == 5) {
             OnScore(m_data_container.m_player_data_handler.score_count, GameTypes::eGameSide::PLAYER);
         }

         m_data_container.RewardCounter(m_data_container.m_player_data_handler, m_data_container.TapRewards);
    }
    void GameLoop::OnScore(const int score, const GameTypes::eGameSide)
    {
        ++(m_data_container.m_player_data_handler.score_count);
        if(m_data_container.m_player_data_handler.win_count == 3) {
            OnMatchResult(GameTypes::eGameSide::PLAYER);
        }

        m_data_container.RewardCounter(m_data_container.m_player_data_handler, m_data_container.ScoreRewards);

    }
    void GameLoop::OnMatchResult(const GameTypes::eGameSide)
    {
        m_data_container.RewardCounter(m_data_container.m_player_data_handler, m_data_container.WinRewards);
